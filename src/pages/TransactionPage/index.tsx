...
export const TransactionPage = () => {
	...
	// hash or number
	const [tx, setTx] = useState<RPCStakingTransactionAstra>(
		{} as RPCStakingTransactionAstra
	);
	...
	useEffect(() => {
		const getTxInputSignature = async (trx: RPCStakingTransactionAstra) => {
			let signature;
			try {
				const signatures = await getByteCodeSignatureByHash([
		...
		const getTx = async () => {
			...
			if (trx) {
				const txnReceipt = await astrav2_getTransactionReceipt([id], shard);
			...
			setTx((trx || {}) as RPCStakingTransactionAstra);
