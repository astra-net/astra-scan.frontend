...
export function CheckARC() {
  ...
  const onContractLoaded = (bytecode: string) => {
    const matched = {
      [ContractType.erc20]: erc20ABIManager.getMatchingSignatures(
        methods[ContractType.erc20],
        bytecode
      ),
      [ContractType.erc721]: erc721ABIManager.getMatchingSignatures(
        methods[ContractType.erc721],
        bytecode
      ),
      [ContractType.erc1155]: erc1155ABIManager.getMatchingSignatures(
        methods[ContractType.erc1155],
        bytecode
      ),
    };
    ...
    if (
      matched[ContractType.erc721].length ===
      methods[ContractType.erc721].length
    ) {
      type = ContractType.erc721;
    } else if (
      matched[ContractType.erc1155].length ===
      methods[ContractType.erc1155].length
    ) {
  ...
  useEffect(() => {
    const getContract = async () => {
      ...
      try {
        let contract: any = await getContractsByField([
          0,
          "address",
          contractAddress,
        ]);
        ...
        const message =
          e && (e as Error).message ? (e as Error).message : "Unknown error";
  ...
  const isAllEventsMatched =
    matchedEvents.length === methods[contractType].length;
      ...
      <BasePage pad={"small"} style={{ overflow: "inherit" }}>
        <Box>
          <Box direction="row" justify={"start"} wrap pad={"16px"} gap={"16px"}>
            ...
            {!isContractLoading && contractAddress && (
              <Box justify="center" align="center" margin={"24px 0 0 24px"}>
                {isAllEventsMatched ? (
                  <Tip content={"All events found"}>
                    <StatusGood size="medium" color={"successText"} />
                  </Tip>
                ) : (
                  <Tip content={"Some events missing"}>
        ...
        {contractAddress && (
          <Box direction="column" justify="between" wrap pad={"16px"}>
            <Box>Required events:</Box>
            <EventsWrapper direction={"row"}>
              {methods[contractType].map((name) => {
                const isMatched = !!matchedEvents.find(
                  (matchedName) => matchedName === name
                );
                const signature = geMethodSignature(name);
                return (
                  <Tip content={`Signature: ${signature}`}>
                    <EventContainer
                      onClick={() => onEventClicked(name)}
                      isMatched={isMatched}
                    >
                      {name}
                    </EventContainer>
        ...
        <Box direction="column" justify="between" wrap pad={"16px"}>
          <StyledTextArea
            ...
            rows={7}
            cols={100}
            value={contractData ? contractData.bytecode : ""}
