...
export const AbiMethodsView = (props: {
	...
	const query = async () => {
		try {
			...
			const astraWeb3 = new Web3(web3URL);

			const contract = new astraWeb3.eth.Contract([abiMethod], address);
		...
		<ViewWrapper direction="column" margin={{ bottom: "medium" }}>
			...
			<Box pad="20px">
				{abiMethod.stateMutability === "payable" ? (
					<Field gap="5px">
						<Text size="small">
							payableAmount <span>ASTRA</span>
						</Text>
						<SmallTextInput
							value={amount}
							placeholder={`payableAmount (ASTRA)`}
