...
import { ApprovalDetails, RPCTransactionAstra } from "src/types";
...
export function TokenApprovals(props: { contractAddress?: string }) {
	...
	const loadTransactions = async ({ account = "", contractAddress = "" }) => {
		...
		const contractAstraAddr =
			contractAddress.length > 0 ? new AstraAddress(contractAddress) : null;

		const key = account + (contractAstraAddr ? contractAddress : "");
		const pageSize = 100;
		...
		let result: {
			...
			txnHistory: ApprovalCache.get(account + "-" + key + "-txnHistory") || [],
			dataObj: [],
		};
		try {
			do {
				...
				if (result.dataObj.length > 0) {
					// update only when there is data
