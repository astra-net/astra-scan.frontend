...
import { AstraAddress } from "./getAddress/GetAddress";
...
export const convertTxnToObj = (tx: any, type: TokenType): ApprovalDetails => {
	const assetAddress = new AstraAddress(tx.to).basicHex;
	...
	const approvedObj: ApprovalDetails = {
		...
		action: "action",
		account: new AstraAddress(tx.from).basicHex,
		contract: tx.to,
		type: type,
	};
...
export const matchesApproveAllNonFungible = (
...
export const matchesApprovalMethod = (tx: RPCTransactionAstra): boolean => {
	return (
		tx.input.includes(approval20Hash) ||
...
export const filterTransactions = (
	...
	if (erc20[to]) {
		...
		txnHistory = txnHistory.filter(
			(transaction) =>
				!(transaction.spender === spender && transaction.contract === tx.to)
		);
		return txnHistory;
		...
		if (matchesApproveAllNonFungible(tx)) {
			...
			return txnHistory.filter(
				(transaction) =>
					!(transaction.tokenId === tokenId && transaction.contract === tx.to)
			);
		}
	}
	return txnHistory;
...
export const allowanceAmount = (tx: RPCTransactionAstra): string => {
...
export const revokePermission = async (tx: ApprovalDetails) => {
	...
	const astraWeb3 = new Web3(web3Provider);

	if (tx.type === "ERC20") {
		const contract = new astraWeb3.eth.Contract(
			approvalERC20ABI,
			contractAddress
		);
		...
		if (tx.isFullApproval) {
			const contract = new astraWeb3.eth.Contract(
				approvalNonFungibleABI,
				contractAddress
			);
			await contract.methods
				.setApprovalForAll(spender, false)
				.send({ from: account }); // clear approvals for all
		} else {
			const contract = new astraWeb3.eth.Contract(
				approvalNonFungibleABI,
				contractAddress
			...
			await contract.methods
				.approve(EMPTY_ADDRESS, tx.tokenId)
				.send({ from: account }); // clear approval for tokenId
		}
	}
};
