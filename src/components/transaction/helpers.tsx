...
import { IHexSignature, RPCTransactionAstra } from "../../types";
import {
	Address,
	ASTRAValue,
	BlockHash,
	BlockNumber,
	DateTime, StakingTransactionTypeValue, TransactionHash
} from "../ui";
...
export const transactionPropertyDescriptions: Record<string, string> = {
	...
	hash_astra:
		"A TxHash or transaction hash is a unique 66 characters identifier that is generated whenever a transaction is executed. Shard ID is also involved in calculation of Astra Hash.",
	...
	value: "The value being transacted in ASTRA and fiat value.",
	...
	gasPrice:
		"Cost per unit of gas specified for the transaction, in ASTRA. The higher the gas price the higher chance of getting included in a block.",
...
export const transactionPropertyDisplayValues: any = {
			...
			.map((internalTx, i) => {
				...
				return (
					<div style={{ display: "flex", flexDirection: "row" }}>
						...
						&nbsp;&nbsp;
						<ASTRAValue value={v} timestamp={tx.timestamp} />
						{i > 0 && (
							<>
								&nbsp;
								<Address isShort={true} address={internalTx.from} />
								<div>
									<FormNextLink size="small" color="brand" />
								</div>
								<Address isShort={true} address={internalTx.to} />
								&nbsp;Internal
							</>
						)}
					</div>
		...
		return (
			<span>
				{values.length ? (
					...
					<ASTRAValue value={value} timestamp={tx.timestamp} />
				)}
			</span>
		);
	},
	...
	hash_astra: (value: any) => <TransactionHash hash={value} />,
	blockHash: (value: any) => <BlockHash hash={value} />,
	timestamp: (value: any) => <DateTime date={value} />,
	gasUsed: (value: any, tx: RPCTransactionAstra) => (
		<span>
			{value}
			{/* {value} ({+value / +tx.gas}%){" "} */}
		</span>
	),
	shardID: (value: any, tx: RPCTransactionAstra) => (
		<span>
			{value}
			<FormNextLink size="small" color="brand" />
			{tx.toShardID}
	...
	amount: (value: any, tx: any, internalTxs: any[]) => {
		return <ASTRAValue value={value} timestamp={tx.timestamp} />;
	},
	...
	) => <TxInput input={tx.input} inputSignature={inputSignature} />,
};

export const transactionDisplayValues = (
	transaction: RPCTransactionAstra,
	key: string,
	value: any,
	type: string,
	internalTxs: any[] = [],
